...
def EmbeddingSimulator(x, rhoP1, rhoM1, m, fixEmbeddingChanges):
    ...
    y[np.logical_and(randChange >= pChangeP1, randChange < (pChangeP1 + pChangeM1))] = y[np.logical_and(
        randChange >= pChangeP1, randChange < (pChangeP1 + pChangeM1))] - 1
...
if __name__ == '__main__':
    cover_path = r'D:\Work\dataset\steganalysis\BOSSbase_1.01_256'
    stego_path = r'D:\Work\dataset\steganalysis\BOSSbase_256_WOW'
    ...
    for index, img_path in enumerate(files_in_dir):
        ...
        file_name = common_utils.get_file_name_from_path(img_path)
        stego_file_path = os.path.join(stego_path, file_name)
        # imageio.imsave(stego_file_path, np.uint8(stego[0, :, :, 0]))
        imageio.imsave(stego_file_path, np.uint8(stego))
        # imageio.imsave(stego_file_path, stego)
        # misc.imsave(stego_file_path, stego)
        if index % 100 == 0:
            print("{}/{}".format(index, file_length))
